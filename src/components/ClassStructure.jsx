...
export default function ClassStructure({
  ...
  const resolvedStructure = useMemo(() => {
    ...
    return {
      ...
      treasurers: Array.isArray(structure?.treasurers) ? structure.treasurers : [
  ...
  // Filtered divisions for search in view mode
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-slate-300 text-xs sm:text-sm max-w-xl leading-relaxed">
              Daftar susunan pengurus inti ruang kelas: Komti, Wakil Komti, Bendahara, serta Kepala Divisi dan Penanggung Jawab (PJ) kelas {currentClass?.name || ''}.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 shrink-0">
            {canEdit && (
              <button
                ...
                className="w-full sm:w-auto px-5 py-2.5 rounded-2xl bg-white text-slate-900 hover:bg-slate-100 font-bold text-xs sm:text-sm flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95 cursor-pointer"
              >
                <Edit3 size={16} className="text-indigo-600" />
                <span>Edit Struktur Kelas</span>
              </button>
            )}
          </div>
        </div>
